...
# v1 is release v0.0.4 and prior, v1.1 changed btn to kb_btn, v2 is release v0.0.5, v2.1 added Hide IPS, 2.2 added strum waterfall effect toggle
...
func post_init():
	current_config.set_waterfall_strums(config_file.get_value("Settings", "waterfall_strums", false))
...
func save_config(version: float=2.2):
	...
	new_config_file.set_value("Settings", "waterfall_strums", current_config.waterfall_strums)
	# colors
	save_colors()
...
func save_bindings():
	for btn: InputBtn in Singleton.btns.slice(0,7):
